...
import com.ben.bencustomerserver.manager.BenThreadManager

abstract class BenBasePresenter : LifecycleObserver {
    /**
     * 生命周期正在销毁
     *
     * @return
    ...
    fun runOnUI(runnable: Runnable?) {
        BenThreadManager.instance?.runOnMainThread(runnable)
    }
    ...
    fun runOnIO(runnable: Runnable?) {
        BenThreadManager.instance?.runOnIOThread(runnable)
    }
    ...
    companion object {
        private val TAG = BenBasePresenter::class.java.simpleName
    }
}