...
object BenFileUtils {
    private val TAG = BenFileUtils::class.java.simpleName
    ...
    fun isFileExistByUri(context: Context?, fileUri: Uri?): Boolean {
        return BenFileHelper.instance.isFileExist(fileUri)
    }
    ...
    fun getFileNameByUri(context: Context?, fileUri: Uri?): String {
        return BenFileHelper.instance.getFilename(fileUri)
    }
    ...
    fun getFilePath(context: Context?, uri: Uri?): String {
        return BenFileHelper.instance.getFilePath(uri)
    }
    ...
    fun saveUriPermission(context: Context?, fileUri: Uri?, intent: Intent?): Boolean {
        ...
        try {
            ...
            Log.e("BenFileUtils", "saveUriPermission failed e: " + e.message)
        }
    ...
    fun takePersistableUriPermission(context: Context?, uri: Uri?): Uri? {
        ...
        if (!TextUtils.isEmpty(last)) {
            ...
            val fileUri = ""
            //            String fileUri = BenPreferenceManager.getInstance().getString(last);
            if (!TextUtils.isEmpty(fileUri)) {
                return try {
                    ...
                    Log.e("BenFileUtils", "takePersistableUriPermission failed e: " + e.message)
                    null
                }
            }
        }
