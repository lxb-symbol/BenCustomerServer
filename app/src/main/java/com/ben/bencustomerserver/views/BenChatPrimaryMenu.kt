...
import com.ben.bencustomerserver.views.BenInputEditText.OnEditTextChangeListener

/**
 * 包含对话框，发送按钮，语音按钮的具体实现
 *
 */
class BenChatPrimaryMenu @JvmOverloads constructor(
    context: Context,
    ...
    override var editText: BenInputEditText? = null
    ...
    private var listener: BenChatPrimaryMenuListener? = null
    private var menuType = BenInputMenuStyle.All//菜单展示形式
    private var inputManager: InputMethodManager
    private var activity: Activity

    init {
        LayoutInflater.from(context).inflate(R.layout.ben_widget_chat_primary_menu, this)
    ...
    override fun setMenuShowType(style: BenInputMenuStyle?) {
        menuType = style!!
        checkMenuType()
    }
    ...
    override fun setBenChatPrimaryMenuListener(listener: BenChatPrimaryMenuListener?) {
    ...
    private fun checkMenuType() {
        when (menuType) {
            BenInputMenuStyle.DISABLE_VOICE -> {
                buttonSetModeVoice!!.visibility = GONE
                buttonSetModeKeyboard!!.visibility = GONE
                buttonPressToSpeak!!.visibility = GONE
            }
            BenInputMenuStyle.DISABLE_EMOJICON -> {
                faceLayout!!.visibility = GONE
            }
            BenInputMenuStyle.DISABLE_VOICE_EMOJICON -> {
                buttonSetModeVoice!!.visibility = GONE
                buttonSetModeKeyboard!!.visibility = GONE
                buttonPressToSpeak!!.visibility = GONE
                faceLayout!!.visibility = GONE
            }
            BenInputMenuStyle.ONLY_TEXT -> {
                buttonSetModeVoice!!.visibility = GONE
                buttonSetModeKeyboard!!.visibility = GONE
                buttonPressToSpeak!!.visibility = GONE
                faceLayout!!.visibility = GONE
