...
import com.ben.bencustomerserver.manager.BenVoiceRecorder
import com.ben.bencustomerserver.utils.BenCommonUtils.isSdcardExist
 ...
 *
 */
class BenVoiceRecorderView : RelativeLayout {
     lateinit var micImages: Array<Drawable>
     var voiceRecorder: BenVoiceRecorder? = null
    ...
    private fun init(context: Context) {
        LayoutInflater.from(context).inflate(R.layout.ben_widget_voice_recorder, this)
        ...
        voiceRecorder = BenVoiceRecorder(micImageHandler)

        // animation resources, used for recording
        micImages = arrayOf(
            resources.getDrawable(R.drawable.ben_record_animate_01),
            resources.getDrawable(R.drawable.ben_record_animate_02),
            resources.getDrawable(R.drawable.ben_record_animate_03),
            resources.getDrawable(R.drawable.ben_record_animate_04)
        )
    ...
    fun onPressToSpeakBtnTouch(
        v: View,
        event: MotionEvent,
        recorderCallback: BenVoiceRecorderCallback?
    ): Boolean {
        return when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                try {
                    val voicePlayer = BenChatRowVoicePlayer.getInstance(context!!)
    ...
    interface BenVoiceRecorderCallback {
        /**
    ...
    fun startRecording() {
        ...
        try {
            ...
            ivIcon!!.setImageResource(R.drawable.ben_record_icon)
    ...
    fun showReleaseToCancelHint() {
        recordingHint!!.text = context!!.getString(R.string.relben_to_cancel)
        ivIcon!!.setImageResource(R.drawable.ben_record_cancel)
