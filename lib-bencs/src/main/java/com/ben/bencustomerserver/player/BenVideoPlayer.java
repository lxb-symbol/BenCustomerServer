...
import com.ben.bencustomerserver.player.BenIUserMethods;
import com.ben.bencustomerserver.player.BenVideoCallback;
import com.ben.bencustomerserver.player.BenVideoProgressCallback;
 ...
 */
public class BenVideoPlayer extends FrameLayout
        implements BenIUserMethods,
    ...
    private Uri mSource;
    private BenVideoCallback mCallback;
    private BenVideoProgressCallback mProgressCallback;
    ...
    private final Runnable mUpdateCounters =
            new Runnable() {
                ...
                public void run() {
                    ...
                    if ("oppo".equals(Build.BRAND.toLowerCase()) && "OPPO R9sk".equals(Build.MODEL) && pos <= BenVideoPlayer.this.currentPos) {
                        pos = BenVideoPlayer.this.currentPos;
                    }
                    if (pos > dur) pos = dur;
    ...
    public BenVideoPlayer(Context context) {
        super(context);
        init(context, null);
    }

    public BenVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public BenVideoPlayer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a =
                    context.getTheme().obtainStyledAttributes(attrs, R.styleable.BenVideoPlayer, 0, 0);
            try {
                String source = a.getString(R.styleable.BenVideoPlayer_ben_source);
                if (source != null && !source.trim().isEmpty()) mSource = Uri.parse(source);

                int playDrawableResId = a.getResourceId(R.styleable.BenVideoPlayer_ben_playDrawable, -1);
                int pauseDrawableResId = a.getResourceId(R.styleable.BenVideoPlayer_ben_pauseDrawable, -1);
                ...
                mControlsDisabled = a.getBoolean(R.styleable.BenVideoPlayer_ben_disableControls, false);
        ...
        if (mPlayDrawable == null)
            mPlayDrawable = AppCompatResources.getDrawable(context, R.drawable.ben_action_play);
        if (mPauseDrawable == null)
            mPauseDrawable = AppCompatResources.getDrawable(context, R.drawable.ben_action_pause);
    }
    ...
    @Override
    public void setCallback(@NonNull BenVideoCallback callback) {
        mCallback = callback;
    }

    @Override
    public void setProgressCallback(@NonNull BenVideoProgressCallback callback) {
    ...
    public void setPlayDrawableRes(@DrawableRes int res) {
        setPlayDrawable(Objects.requireNonNull(AppCompatResources.getDrawable(getContext(), res)));
    }
    ...
    public void setPauseDrawableRes(@DrawableRes int res) {
        setPauseDrawable(Objects.requireNonNull(AppCompatResources.getDrawable(getContext(), res)));
    }
            ...
            mClickFrame.setOnClickListener(
                    new OnClickListener() {
                        ...
                        public void onClick(View view) {
                            toggleControls();
                            if (mCallback != null) {
                                mCallback.onClickVideoFrame(BenVideoPlayer.this);
                            }
                        }
                    });
        }
