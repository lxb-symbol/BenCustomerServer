...
class BenEmojiconPagerView @JvmOverloads constructor(
    private val context: Context,
    attrs: AttributeSet? = null
) : ViewPager(
    context, attrs
) {
    private var groupEntities: List<BenEmojiconGroupEntity>? = null
    ...
    private var pagerViewListener: BenEmojiconPagerViewListener? = null
    private var viewpages: MutableList<View>? = null
    fun init(
        emojiconGroupList: List<BenEmojiconGroupEntity>?,
        emijiconColumns: Int,
    ...
    fun setPagerViewListener(pagerViewListener: BenEmojiconPagerViewListener?) {
     ...
     * @return
     */
    private fun getGroupGridViews(groupEntity: BenEmojiconGroupEntity): List<View> {
        ...
        if (emojiType == BenEmojicon.Type.BIG_EXPRESSION) {
        ...
        for (i in 0 until pageSize) {
            val view = inflate(context, R.layout.ben_expression_gridview, null)
            ...
            val list: MutableList<BenEmojicon?> = ArrayList()
            if (i != pageSize - 1) {
            ...
            gv.onItemClickListener =
                AdapterView.OnItemClickListener { parent: AdapterView<*>?, view1: View?, position: Int, id: Long ->
                    ...
                    if (pagerViewListener != null) {
                        ...
                        if (emojiText != null && emojiText == BenSmileUtils.DELETE_KEY) {
     ...
     * @param groupEntity
     */
    fun addEmojiconGroup(groupEntity: BenEmojiconGroupEntity, notifyDataChange: Boolean) {
     ...
     * @return
     */
    private fun getPageSize(groupEntity: BenEmojiconGroupEntity): Int {
        val emojiconList = groupEntity.emojiconList
    ...
    interface BenEmojiconPagerViewListener {
        /**
        ...
        fun onExpressionClicked(emojicon: BenEmojicon?)
    }
}